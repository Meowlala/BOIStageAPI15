...
---Split passed list of rooms depending on specified logic, remember to clear lists generated by this
-- on mod reload with `StageAPI.ClearSplitRoomLists` before reloading the mod, with the same parameters.
...
function StageAPI.ClearSplitRoomLists(roomsList, splitBy, splitByType, listNamePrefix)
    listNamePrefix = listNamePrefix or ""
    local roomPairs
    if roomsList[1] and roomsList[1].TYPE then
        roomsList = {roomsList}
    ...
    if roomPairs then
        for listName, _ in pairs(roomPairs) do
            StageAPI.RoomsLists[listNamePrefix .. listName] = nil
        end
    end
end
